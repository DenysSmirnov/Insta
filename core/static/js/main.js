...
if (window.location.pathname === '/') {
    ...
    $.ajax({
        ...
        if (data.length > 0) {
            ...
            let article = `<article class='_s5vjd _622au _5lms4 _8n9ix'>
<header class='_art_head'><a class='_4a6q9 _i2o1o _gvoze' href='/${img.author.name}/' style='width: 30px;height: 30px;'>
<img class='_rewi8' src='${conf.uploadUrl}${img.author.avatar}'></a><div><a class='_art_head_2' href='/${img.author.name}/'>
${img.author.name}</a><div>${img.title}</div></div>${del_post_btn_if_author(img.author.name, img._id.$oid)}</header>
<div><img src='${conf.uploadUrl}${img.path}' width='600px'></div><div class='_art_foot'>${getLike(img.liked_users, img._id.$oid)}
<div class='_com _aut_com'><span>${getTag(img.author.name, img.description, img.tags)}</span></div>
<div><div class='_listcoms'>${getCom(img.comments, img.author.name)}</div><div class='_ha6c6 _6d44r'>
<time class='_p29ma _6g6t5' title='${moment(img.created_time.$date).format('LL')}'>${moment(img.created_time.$date).fromNow()}
...
var item_target = item.filter(function() {
    return $(this).index() > 2;
    });
...
$.ajax({
    ...
    btn.closest("._art_foot").find("._listcoms").append(
        `<div class='_hdiv _com _aut_com'><a class='_art_head_2' href='/${conf.uname}/'>${conf.uname}</a>
